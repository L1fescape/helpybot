...
module.exports = Class.extend({
  ...
  getUser: function(userID) {
    ...
    return this.slack.getUserByID(userID) || this.slack.getUserByName(userID);
  },

  command: function(regex, helpText, fn, rawText) {
    this.respond('command', regex, helpText, fn, rawText);
  },

  hear: function(regex, helpText, fn, rawText) {
    this.respond('hear', regex, helpText, fn, rawText);
  },
  ...
  message: function(message) {
    ...
    var isMentioned = util.isBotMentioned(text, aliases);
    var isDirectMessage = message.channel[0] === 'D';
  ...
  checkCommands: function(command, text, channel, user) {
    ...
    _.each(this.triggers.oncommand, function(obj) {
      ...
      if (match && match.length) {
        obj.command(command, channel, user, text);
      }
    });
  },
  ...
  checkHeard: function(text, channel, user) {
    ...
    _.each(this.triggers.onhear, function(obj) {
      ...
      if (match && match.length) {
        obj.command(text, channel, user, raw);
      }
    });
  }
});
